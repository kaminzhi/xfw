...
#[test]
fn test_style_tree_with_children() {
    let child1 = StyleNode::container(Some("child1".to_string()), TaffyStyle::default(), vec![]);
    let child2 = StyleNode::container(Some("child2".to_string()), TaffyStyle::default(), vec![]);
    let root = StyleNode::container(
        Some("root".to_string()),
        TaffyStyle::default(),
        vec![child1, child2],
    );
    let tree = StyleTree::new(root);
    assert_eq!(tree.node_count(), 3);
}

#[test]
fn test_find_by_id() {
    let child = StyleNode::container(Some("target".to_string()), TaffyStyle::default(), vec![]);
    let root = StyleNode::container(Some("root".to_string()), TaffyStyle::default(), vec![child]);
    let tree = StyleTree::new(root);
...
#[test]
fn test_find_by_prefix() {
    let node1 = StyleNode::container(Some("battery".to_string()), TaffyStyle::default(), vec![]);
    let node2 = StyleNode::container(
        Some("battery_level".to_string()),
        TaffyStyle::default(),
        vec![],
    );
    let node3 = StyleNode::container(Some("volume".to_string()), TaffyStyle::default(), vec![]);
    let root = StyleNode::container(
        Some("root".to_string()),
        TaffyStyle::default(),
        vec![node1, node2, node3],
...
#[test]
fn test_text_node() {
    let node = StyleNode::text(
        Some("text1".to_string()),
        TaffyStyle::default(),
        "Hello".to_string(),
    );
    let tree = StyleTree::new(node);

    let found = tree.find_by_id("text1").unwrap();
    if let StyleNode::Text { content, .. } = found {
        assert_eq!(content, "Hello");
    } else {
        panic!("Expected Text node");
    }
...
fn test_image_node() {
    ...
    let found = tree.find_by_id("img1").unwrap();
    if let StyleNode::Image { path, .. } = found {
        assert_eq!(path, "/path/to/image.png");
    } else {
        panic!("Expected Image node");
    }
