...
use wayland_protocols::wlr::unstable_layer_shell::v1::client::{
...
impl Anchor {
    pub fn to_wl(self) -> u32 {
        match self {
            Anchor::Top => zwlr_layer_surface_v1::Anchor::Top.bits(),
            Anchor::Bottom => zwlr_layer_surface_v1::Anchor::Bottom.bits(),
            Anchor::Left => zwlr_layer_surface_v1::Anchor::Left.bits(),
            Anchor::Right => zwlr_layer_surface_v1::Anchor::Right.bits(),
            Anchor::TopLeft => {
                zwlr_layer_surface_v1::Anchor::Top.bits()
                    | zwlr_layer_surface_v1::Anchor::Left.bits()
            }
            Anchor::TopRight => {
                zwlr_layer_surface_v1::Anchor::Top.bits()
                    | zwlr_layer_surface_v1::Anchor::Right.bits()
            }
            Anchor::BottomLeft => {
                zwlr_layer_surface_v1::Anchor::Bottom.bits()
                    | zwlr_layer_surface_v1::Anchor::Left.bits()
            }
            Anchor::BottomRight => {
                zwlr_layer_surface_v1::Anchor::Bottom.bits()
...
impl From<KeyboardInteractivity> for zwlr_layer_shell_v1::KeyboardInteractivity {
    fn from(interactivity: KeyboardInteractivity) -> Self {
        match interactivity {
            KeyboardInteractivity::None => zwlr_layer_shell_v1::KeyboardInteractivity::None,
            KeyboardInteractivity::Exclusive => {
                zwlr_layer_shell_v1::KeyboardInteractivity::Exclusive
            }
            KeyboardInteractivity::OnDemand => zwlr_layer_shell_v1::KeyboardInteractivity::OnDemand,
        }
    }
}
...
pub struct LayerSurface {
    ...
    pub layer_surface: Proxy<zwlr_layer_surface_v1::ZwlrLayerSurfaceV1>,
...
impl LayerSurface {
    pub fn new(connection: &WaylandConnection, config: LayerSurfaceConfig) -> Result<Self> {
        ...
        let layer_surface = layer_shell
            ...
            .map_err(|e| {
                PlatformError::Surface(format!("Failed to create layer surface: {}", e))
            })?;

        layer_surface
            .set_anchor(config.anchor.to_wl())
            .map_err(|e| PlatformError::Surface(format!("Failed to set anchor: {}", e)))?;
        ...
        layer_surface
            ...
            .map_err(|e| PlatformError::Surface(format!("Failed to set margin: {}", e)))?;

        layer_surface
            .set_keyboard_interactivity(u32::from(config.keyboard_interactivity))
            .map_err(|e| PlatformError::Surface(format!("Failed to set keyboard: {}", e)))?;
